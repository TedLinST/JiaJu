/**
 * Created by Z.cr on 2018-2-9.
 * TODO: 左边表格因为 overflow-y 样式设置为 hidden，不能触发垂直滚动事件，无法实现表格滚动( mousewheel 事件 )
 * 分栏表格，继承于 KeyTableComponent 控件
 *
 * 基本用例
 * <key-table-split title="分栏表格信息" [(selection)]="selection" [data]="data"
 *    (refresh)="list($event)">
 ...
 *         <key-table-editor-text></key-table-editor-text>
 *     </key-table-column>
 *     <key-table-column heading="列项 - 筛选、锁定功能" key="key5" width="170" [lock]="true" sortable>
 *         <key-table-filter-text></key-table-filter-text>
 *     </key-table-column>
 * </key-table-split>
 */
...
@Component({
    selector: "key-table-split",
    templateUrl: "./table-split.component.html"
})
export class KeyTableSplitComponent extends KeyTableComponent implements AfterContentInit {
    ...
    constructor(differs: IterableDiffers, changeDetector: ChangeDetectorRef, element: ElementRef) {
        // 执行父类的构造函数
